...
import { getHistoryForUser } from '../mocks/mockHistory';
...
export default function ProfileScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    if (!user) return;
    const load = async () => {
      const stored = await AsyncStorage.getItem(`gameHistory_${user.id}`);
      const localHistory: GameRecord[] = stored ? JSON.parse(stored) : [];
      setHistory([...localHistory, ...getHistoryForUser(user.id)]);
    };
    load();
  }, [user]);
  ...
  const handleSignOut = async () => {
    await signOut();
    // pas besoin de navigation.navigate : AppNavigator bascule automatiquement
    // vers Login dès que `user` devient null
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>Aucune partie jouée pour l'instant 😿</Text>}
      />
...
const styles = StyleSheet.create({
  ...
  profileCard: { backgroundColor: Colors.surface, marginHorizontal: 20, borderRadius: 20, padding: 24, alignItems: 'center', borderWidth: 1, borderColor: Colors.surfaceLight, marginBottom: 20 },
  avatar: { fontSize: 64, marginBottom: 8 },
  pseudo: { fontSize: 24, fontWeight: '800', color: Colors.amber, letterSpacing: 1 },
  ...
  historyRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.surface, borderRadius: 12, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: Colors.surfaceLight },
